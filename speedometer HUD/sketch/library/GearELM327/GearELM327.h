...
public:
   ...
   * 		 * - °C
   		 */
  byte MAFAirFlowRate(int &rate);
  byte MAFAirFlowRate(float &rate);  
  ...
  byte controlModuleVoltage(float &voltage);
...
#define OPEN_LOOP_TEMPERATURE 1
#define CLOSED_LOOP 2
#define OPEN_LOOP_LOAD 4
#define OPEN_LOOP_FAILURE 8
#define CLOSE_LOOP_FAULT 16

  byte fuelSystemStatus(int &status);
#endif
#ifdef ELM327_DEBUG
  char tmp[100];
#endif
...
public:
  void flush();
  static String debugStatus(byte status);
